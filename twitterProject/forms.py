...
from twitterProject.models import UserManagement
...
class Signup(FlaskForm):
    ...
    def validate_username(self,username):
        user = UserManagement.query.filter_by(username=username.data).first()
        if user:
            raise ValidationError('Username is already taken. Please choose a different one')

    def validate_email(self,email):
        user = UserManagement.query.filter_by(email=email.data).first()
        if user:
...
class ResetRequest(FlaskForm):
    email = StringField('Email',validators=[DataRequired(),Email()])
    password = PasswordField('Password',validators=[DataRequired()])
    sumbit = SubmitField(lables='Reset Password')

class ConformPasswordForm(FlaskForm):
    password = PasswordField('Password',validators=[DataRequired()])
